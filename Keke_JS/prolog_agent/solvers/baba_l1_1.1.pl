...
% *** solved(You, WinCond) *****************
% *****************************************
can(won(You,Phys), [rule(You,you), rule(Phys,win), phys(You), phys(Phys), free(Phys,You)], baba).
...
adds(destroyRule(Phys, Rule),[removed(Phys)], _, baba):- % Ostacolo rimosso - per chi? serve "You"
    phys(Phys),
	word(Phys),
    word(Rule).
...
adds(unbond(Phys1, Rule, Phys2),[removed(Phys1)], _, baba). % Ostacolo rimosso - "You"? -> no
...
adds(sink(Phys1, Phys2),[removed(Phys2)], _, baba). % Ostacolo rimosso - "You"? -> No
...
adds(transform(Phys1, Phys2),[phys(Phys2), free(Phys2, baba)], _, baba):- % free(Phys2, You) - metto baba direttamente per semplicità
	word(Phys1),
    word(Phys2),
    phys(Phys1).

deletes(transform(Phys1, Phys2),[phys(Phys1),free(Phys1, baba)], baba):- % free(Phys1, You) - metto baba direttamente per semplicità
	word(Phys1),
    word(Phys2),
    phys(Phys1).
...
can(free(Phys, Obstacle, You),[rule(You,you), obstacle(Obstacle, You), removed(Obstacle), phys(Phys), phys(Obstacle)], baba). % l'ostacolo c'è ma è stato rimosso (distuggendo la regola)

adds(free(Phys, Obstacle, You),[free(Phys, You)], _, baba).

deletes(free(Phys, Obstacle, You),[], baba).

% FREE - PUSHING 
can(push(Obstacle),[rule(You,you), obstacle(Obstacle,You), rule(Obstacle,push), phys(Obstacle)], baba). % ostacolo presente ma si può spostare con una push
...
solve_l1(lvl_04,P) :-
    plan([
        ...
        free(baba,flag), free(grass,love)],
            [won(love,grass)], baba,
            P).
...
solve_l1(lvl_14,P) :-
    ...
    phys(baba), phys(wall), phys(flag), phys(keke),
        ...
        free(flag,keke), obstacle(wall, baba)],
            [won(keke,flag)], baba,
            P).