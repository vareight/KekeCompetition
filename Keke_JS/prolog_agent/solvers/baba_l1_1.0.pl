...
can(free(Phys, Obstacle),[obstacle(Obstacle), removed(Obstacle), phys(Phys), phys(Obstacle)], baba). % l'ostacolo c'è ma è stato rimosso (distuggendo la regola)

adds(free(Phys, Obstacle),[free(Phys)], _, baba).

deletes(free(Phys, Obstacle),[], baba).
