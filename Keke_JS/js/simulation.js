...
session.consult("prolog/graphplan.pl", {
  success: function () {
    console.log("graphplan ok");
  },
  error: function (err) {
    console.log(err);
  },
...
session.consult(program, {
  success: function () {
    session.query(`consult('prolog/baba_v2.1.pl').`, {
      success: function () {
        //session.answers(show);
        //display(answer);
      },
...
function doubleMap2Str(om, bm) {
  ...
  for (var r = 0; r < om.length; r++) {
    for (var c = 0; c < om[0].length; c++) {
      ...
      else str += reverseChar(o.name + (o.type == "word" ? "_word" : "_obj"));
    }
    str += "\n";
  }
  str = str.replace(/\n$/, "");
...
function clearLevel(state) {
  ...
  for (let a = 0; a < props.length; a++) {
    ...
    if (Array.isArray(entry)) state[props[a]] = [];
    else state[props[a]] = {};
  }
}
...
function has(arr, ss) {
  for (var i = 0; i < arr.length; i++) {
    if (arr[i].includes(ss)) return true;
  }
  return false;
}
...
function destroyObjs(dead, state) {
  ...
  //reset the objects
  if (dead.length > 0) resetAll(state);
}
...
function win(players, winnables) {
  for (var p = 0; p < players.length; p++) {
    for (var w = 0; w < winnables.length; w++) {
      if (overlapped(players[p], winnables[w])) return true;
    }
  }
  return false;
}
...
function transformation(om, bm, rules, sort_phys, phys) {
  ...
  for (var r = 0; r < rules.length; r++) {
    ...
    if (parts[0] == parts[2] && parts[0] in sort_phys) xisx.push(parts[0]);
  }
  ...
  for (var r = 0; r < rules.length; r++) {
    ...
    //turn all objects
    if (
      xisx.indexOf(parts[0]) == -1 &&
      isObjWord(parts[0]) &&
      isObjWord(parts[2])
    ) {
      // && Object.keys(sort_phys).includes(parts[2])){
...
function changeSprite(o, w, om, bm, phys, sort_phys) {
  ...
  if (objAtPos(o.x, o.y, om) == o) om[o.y][o.x] = o2;
  else bm[o.y][o.x] = o2;
...
function setFeatures(state) {
  ...
  for (var r = 0; r < rules.length; r++) {
    ...
    if (
      features.indexOf(parts[2]) != -1 &&
      features.indexOf(parts[0]) == -1 &&
      parts[0] in sort_phys
    ) {
...
function nextMove(nextDir, state) {
  ...
  if (nextDir != "space") movePlayers(nextDir, moved_objects, state);
...
function canMove(e, dir, om, bm, movedObjs, p, u, phys, sort_phys) {
  //p=players, u=pushables
  if (movedObjs.indexOf(e) != -1) return false;
  ...
  if (o == " ")
    //empty space so movable
    return true;
  if (o.is_stopped)
    //immovable object above it
    return false;
  if (o.is_movable) {
    //move the object next to it
    //console.log(p.indexOf(o) != -1);
    //console.log(p);
    //console.log(o)

    if (o.is_movable && u.indexOf(o) != -1)
      //pushables
      return moveObj(o, dir, om, bm, movedObjs, p, u);
    else if (p.indexOf(o) != -1 && p.indexOf(e) == -1)
      //player
      return true;
    else if (o.type == "phys" && (u.length == 0 || u.indexOf(o) == -1))
      //movable but not pushable
      return false;
    else if (
      (e.is_movable || o.is_movable) &&
      e.type == "phys" &&
      o.type == "phys"
    )
      //automover
      return true;
    else if (e.name == o.name && p.indexOf(e) != -1 && isPhys(o) && isPhys(e))
      //same type of entity = merge
  ...
  if (!o.is_stopped && !o.is_movable) return true;
...
function moveObj(o, dir, om, bm, movedObjs, p, u, phys, sort_phys) {
  ...
  if (canMove(o, dir, om, bm, movedObjs, p, u, phys, sort_phys)) {
    ...
    om[o.y][o.x] = " ";
    if (dir == "up") o.y--;
    else if (dir == "down") o.y++;
    else if (dir == "left") o.x--;
    else if (dir == "right") o.x++;
    om[o.y][o.x] = o;
...
function translateSol(solStr) {
  ...
  for (let p = 0; p < parts.length; p++) {
    let a = parts[p];
    if (a == "U") solution.push("up");
    else if (a == "D") solution.push("down");
    else if (a == "L") solution.push("left");
    else if (a == "R") solution.push("right");
    else if (a == "S") solution.push("space");
  }
  return solution;
}
