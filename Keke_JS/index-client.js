...
function useLastSetup() {
  ...
  else localStorage.lvlSet = document.getElementById("levelSetList").value;

  //set the agent dropdown + value
  if (localStorage.agent) {
    document.getElementById("agentList").value = localStorage.agent;
  } else localStorage.agent = document.getElementById("agentList").value;
}
...
function solvingLevelRow(id) {
  let row = document.getElementById("levelRow" + id);
  if (row) row.classList.add("pendingLevel");
}
...
function pendEllipse() {
  ...
  for (let i = 0; i < pr.length; i++) {
    pr[i].getElementsByClassName("solveStat")[0].innerHTML =
      "[ solving" + estr + " ]";
  }
}
...
function resetLevelTable() {
  let rows = document
    .getElementById("level-table")
    .getElementsByClassName("row");
  for (let i = 0; i < rows.length; i++) {
    //skip header row
    if (rows[i].classList.contains("header")) continue;
...
function updateStats() {
  ...
  let iterCtAvg =
    iterCt.length > 0 ? iterCt.reduce((a, b) => a + b) / iterCt.length : 0;
  let timeCtAvg =
    timeCt.length > 0 ? timeCt.reduce((a, b) => a + b) / timeCt.length : 0;
  let solCtAvg =
    solCt.length > 0 ? solCt.reduce((a, b) => a + b) / solCt.length : 0;
  ...
  document.getElementById("level_acc").innerHTML =
    (levAcc * 100).toFixed(1) + "%";
...
function checkALS() {
  socket.emit("als_exists", {
    agent: localStorage.agent,
    levelSet: localStorage.lvlSet,
  });
}
...
function pauseAgent() {
  ...
  if (
    !confirm(
      `Want to stop running agent [ ${agent} ] on level set [ ${lvlSet} ]?\nAny currently running levels will be finished.`
    )
  ) {
    return;
  }
...
function solveNextLevel() {
  ...
  socket.emit("solve-level", {
    agent: agent,
    levelSet: lvlSet + "_LEVELS",
    levelID: lv,
  });
}
...
function getUnsolvedLevels() {
  ...
  for (let i = 0; i < lvls.length; i++) {
    ...
    //ignore header row
    if (l.classList.contains("header")) continue;
...
function resetAgentData() {
  ...
  if (
    confirm(
      `Are you sure you want to clear the JSON data for [ ${agent} ] on level set [ ${lvlSet} ]?`
    )
  ) {
    //alert("lol, baka");
    socket.emit("delete-json-set", {
      agent: agent,
      levelSet: lvlSet + "_LEVELS",
    });
  }
