...
function sendReport(dat) {
  ...
  if (rep != null) {
    ...
    console.log(
      `- No JSON found for agent [ ${dat["agent"]} ] ON LEVEL SET [ ${dat["levelSet"]} ]... - `
    );
  }
}

// SOLVE A LEVEL BY ID # FOR A LEVEL SET
async function solveLevel(ls, id, agent) {
  io.emit("pending-level", id);
  let res = await execjs.solveLevel(ls, id, agent);
...
io.on("connection", (socket) => {
  ...
  socket.on("solve-level", (dat) => {
    console.log(
      `-- SOLVING LEVEL [ ${dat["levelID"]} ] FROM LEVEL SET [ ${dat["levelSet"]} ] WITH AGENT [ ${dat["agent"]} ] --`
    );
  ...
  socket.on("delete-json-set", (dat) => {
    ...
    console.log(
      `-- DELETED REPORT FOR [ ${dat["agent"]} ] ON LEVEL SET [ ${dat["levelSet"]} ] --`
    );
  ...
  socket.on("als_exists", (dat) => {
    if (!jsonjs.agent_exists(dat["agent"])) {
      io.emit("agent-404");
    }
    if (!jsonjs.level_set_exists(dat["levelSet"])) {
      io.emit("level-set-404");
    }
    io.emit("set-als");
  });
