...
% *** solved(You, WinCond) *****************
% *****************************************
can(solveLevel(You,Phys), [rule(You,you), rule(Phys,win), phys(You), phys(Phys)], baba).

adds(solveLevel(You,Phys), [won(You,Phys)], _, baba). % NB: posso mettere anche won() senza argomenti, ma nei livelli complicati mi dà tutte le possibilità. Meglio delegare questo all'esecutore

deletes(solveLevel(You,Phys), [], baba).


% **********************************************
% *** createRule(Word1, Connector, Word2) *****
% **********************************************
can(createRule(Phys, Rule),[word(Phys), word(Rule), phys(Phys)], baba).

adds(createRule(Phys, Rule),[rule(Phys,Rule)], _, baba):-
    phys(Phys),
	word(Phys),
    word(Rule).

deletes(createRule(Phys, Rule),[], baba):-
    phys(Phys), 
	word(Phys),
    word(Rule).

deletes(createRule(Phys, Rule),[mutable(Phys)], baba):-
    phys(Phys),
	Rule = Phys.
